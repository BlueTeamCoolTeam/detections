#!/usr/bin/env python3
"""Live re-verification of the FULL corrected candidate pool (959 sites),
using the exact same polymorphic-aware brute-force methodology already
documented in the post. Read-only: fetches and inspects HTML only."""
...
print(f"Checking {len(sites)} sites, 40 workers, single pass...")
...
with open(f"{OUT_PREFIX}verification_results.txt", "w", encoding="utf-8") as f:
    for v in order:
        f.write(f"\n### {v} ({len(buckets.get(v, []))})\n")
...
print(f"\nWrote {OUT_PREFIX}verification_results.txt and {OUT_PREFIX}confirmed.txt")
