#!/usr/bin/env python3
"""
PART B - full 3-pass live re-confirmation of the existing 638-site
"family_b_mamkor_confirmed.txt" list.

Adapted from the already-written checker at
reproduction-log/05_mamkor_confirmed_site_validator.py: reuses its four
decode schemes (cleartext markers, atob+sub-then-XOR, plain single-byte
XOR, 256-byte S-box) and its 20MB read cap (the naive ~600KB default read
size was found in a prior investigation to truncate the injected script out
of many WordPress pages before it could even be searched - the 20MB cap is
load-bearing, do not shrink it).

Usage:
    python mamkor_B_confirm_checker.py <pass_number>

Writes:
    mamkor_B_pass<N>_results.txt   - one line per site: host\tSTATUS\tdetail
(console progress goes to stdout; the caller redirects that to
mamkor_B_pass<N>_console_OUTPUT.txt)
"""
import base64
...
CONFIRMED_LIST = r"C:\Users\bob\Documents\Tools\BlueTeamCoolTeam\detections\campaigns\etherhiding-ecosystem-mapped\reproduction-log\family_b_mamkor_confirmed.txt"
    ...
    for scheme in ("https", "http"):
        ...
        try:
            ...
            try:
                ...
                last_err = str(e2)
                continue
    return host, None, last_err if 'last_err' in dir() else "unreachable"
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python mamkor_B_confirm_checker.py <pass_number>")
        sys.exit(1)
    pass_num = sys.argv[1]

    hosts = [l.strip() for l in open(CONFIRMED_LIST, encoding="utf-8") if l.strip()]
    print(f"=== PASS {pass_num} ===")
    print(f"Start (UTC): {datetime.now(timezone.utc).isoformat()}")
    print(f"Validating {len(hosts)} confirmed sites, {MAX_WORKERS} workers, {TIMEOUT}s timeout, {READ_CAP} byte read cap...")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
        ...
        for fut in concurrent.futures.as_completed(futures):
            ...
            results.append(res)
            done_count += 1
            if done_count % 25 == 0 or done_count == len(hosts):
                elapsed = time.time() - t0
                print(f"  ...{done_count}/{len(hosts)}  ({elapsed:.0f}s elapsed)")

    # preserve original input order in the output file
    order = {h: i for i, h in enumerate(hosts)}
    results.sort(key=lambda r: order.get(r["host"], 10**9))

    out_path = f"{OUT_DIR}\\mamkor_B_pass{pass_num}_results.txt"
    ...
    elapsed = time.time() - t0
    print(f"\n=== PASS {pass_num} summary ({elapsed:.0f}s total) ===")
